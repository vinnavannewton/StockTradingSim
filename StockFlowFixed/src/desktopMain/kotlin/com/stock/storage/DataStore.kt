...
actual object DataStore {
    private val file: File
        get() = File(System.getProperty("user.home"), ".stockflow/userdata.txt")
    ...
    actual fun load(defaultBalance: Double): User {
        val f = file
        if (!f.exists()) return User(defaultBalance)
        ...
        try {
            FileReader(f).buffered().useLines { lines ->
                lines.forEach { line ->
                    when {
                        ...
                        line.startsWith("TX=") -> {
                            ...
                            if (parts.size == 5) {
                                ...
                                transactions.add(Transaction(type, parts[1], parts[2].toIntOrNull() ?: return@forEach, parts[3].toDoubleOrNull() ?: return@forEach, parts[4].toLongOrNull() ?: return@forEach))
                            }
                        }
                    }
                }
            }
        } catch (_: Exception) { return User(defaultBalance) }
